<template>
  ...
  <div>
    ...
    <el-table v-loading="listLoading" ref="multipleTable" :data="absenceDetailList" border fit highlight-current-row
...
<script>
  ...
  import {
    absenceDetailList,exportAbsenceDetailList
  ...
  export default {
    ...
    methods: {
      ...
      exportExcel() {
        exportAbsenceDetailList(this.listQuery).then(res => {
          console.log("导出。。。")
      ...
      initList() {
        ...
        absenceDetailList(this.listQuery).then(res => {
          if (res.data.code === '0000') {
            this.absenceDetailList = res.data.data
            /*this.total = res.data.data.total
            if (this.absenceDetailList.length === 0 && this.total > 0) {
