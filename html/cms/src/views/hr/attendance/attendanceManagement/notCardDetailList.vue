<template>
  ...
  <div>
    ...
    <el-table v-loading="listLoading" ref="multipleTable" :data="notCardDetailList" border fit highlight-current-row
...
<script>
  ...
  import {
    notCardDetailList,exportNotCardDetailList
  ...
  export default {
    ...
    methods: {
      ...
      exportExcel() {
        exportNotCardDetailList(this.listQuery).then(res => {
          console.log("导出。。。")
      ...
      initList() {
        ...
        notCardDetailList(this.listQuery).then(res => {
          if (res.data.code === '0000') {
            this.notCardDetailList = res.data.data
            /*this.total = res.data.data.total
            if (this.notCardDetailList.length === 0 && this.total > 0) {
