<template>
  <el-dialog :title="title" :visible.sync="dialogVisible" :modal-append-to-body="false"
    ...
    <div>
      ...
      <div class="download-template" v-if="templatePath!=''">
        <a class="btn-download" :href="templatePath">下载模板</a>
      </div>
      <div :class="{'import-failure': importFlag === 2, 'hide-dialog': importFlag !== 2}">
        <div class="failure-reason">
          <h4><i class="el-icon-warning"></i>&nbsp;文件导入失败,原因</h4>
          <ul>
  ...
  export default {
    ...
    methods: {
      ...
      fileChange(file, fileList){
        ...
        let fileExtend = file.name.substring(file.name.lastIndexOf('.')+1).toLowerCase();
