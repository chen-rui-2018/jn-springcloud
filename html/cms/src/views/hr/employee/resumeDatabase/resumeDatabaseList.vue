  ...
  <div class="resumeDatabaseList">
    <el-form :model="listQuery" class="info-form">
      <el-form-item label="学历:" lable-position="right" label-width="129px">
        <el-button-group>
          ...
          <el-button :type="listQuery.educationId==item.key?'primary':''" v-for="item in educationList" :key="item.key"
      ...
      <el-form-item>
        ...
        <el-input v-model="listQuery.name" maxlength="20" style="width:200px;float:right;margin-right:10px;"
                  placeholder="请输入姓名" class="filter-item"
                  clearable @keyup.enter.native="handleFilter"/>
      </el-form-item>

    </el-form>
    <!-- 表格 -->
    <el-table v-loading="listLoading" ref="multipleTable" :data="resumeDatabaseList" border fit highlight-current-row
      ...
      <el-table-column label="操作" align="center" min-width="100" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="text" @click="pass(scope.row)" v-if="scope.row.applicationResult!=1">通过</el-button>
          <el-button type="text" @click="notPass(scope.row)" v-if="scope.row.applicationResult!=2">淘汰</el-button>
    ...
    <template v-if="bgFormVisible">
      <el-dialog :visible.sync="bgFormVisible" title="背景调查" width="450px">
        <el-form ref="bgForm" :rules="rules" :model="bgForm" label-position="right" label-width="130px"
          ...
          <el-form-item label="身份核查:" prop="identityCheck">
            <el-select v-model="bgForm.identityCheck" placeholder="请选择" style="width: 200px">
              ...
              <el-option label="核查属实" value="1"></el-option>
              <el-option label="核查不属实" value="2"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="上家公司名称:" prop="corporateName">
            <el-input v-model.trim="bgForm.corporateName" maxlength="200" style="width: 200px" placeholder="请输入上家公司名称"
                      clearable/>
          </el-form-item>
          <el-form-item label="上家公司职位:" prop="corporateJob">
            <el-input v-model.trim="bgForm.corporateJob" maxlength="200" style="width: 200px" placeholder="请输入上家公司职位"
                      clearable/>
          </el-form-item>
          <el-form-item label="上家公司证明人:" prop="companyCertifier">
            <el-input v-model.trim="bgForm.companyCertifier" maxlength="200" style="width: 200px"
                      placeholder="请输入上家公司证明人" clearable/>
          </el-form-item>
          ...
          <el-form-item label="核查人联系电话:" prop="inspectorsPhone">
            <el-input v-model.trim="bgForm.inspectorsPhone" :rows="2" type="textarea" maxlength="200"
                      style="width: 200px" placeholder="请输入核查人联系电话" clearable/>
          </el-form-item>
        </el-form>
    ...
    <file-upload :title="'简历库导入'" :importUrl="importUrl" :dialogVisible="dialogImportVisible"
                 :templatePath="'/static/file/简历库模板.xlsx'" :limit="1"
...
<script>
  import {
    getResumeDatabaseList, exportResumeDatabase, importResumeDatabase,
  ...
  export default {
    ...
    methods: {
      ...
      downloadResumeDatabase(row) {
        let fileName = row.resumeInfo.substring(row.resumeInfo.lastIndexOf(".") + 1)
