  ...
  <div class="addExamItem">
    <div class="header-steps">
      <el-steps :active="active" simple finish-status="success">
        <el-step title="1  考试信息" />
        <el-step title="2  设计试卷" />
        <el-step title="3  发布考试" />
      </el-steps>
    </div>
    <!--对应三个步骤-->
    <div v-show="active==1" class="stepsBox1">
      ...
      <div>
        <el-header class="el-head">
          ...
          <div class="fr">
            ...
            <el-button class="addAroblem" type="primary" size="small" icon="el-icon-plus" @click="addExam('add')">新增试题</el-button>
          </div>
        </el-header>
        ...
        <el-dialog :visible.sync="dialogFormVisible" title="考试设置">
          <el-form ref="ruleForm2" :model="ruleForm" :rules="rules2" label-width="80px" style="max-width: 500px;">
            <el-form-item label="及格分数" prop="passScore">
              <el-input
                v-model="ruleForm.passScore"
                :min="1"
                :max="ruleForm.totalScore"
                autocomplete="off"
                type="number"
                @change="ruleForm.passScore=ruleForm.passScore>ruleForm.totalScore?ruleForm.totalScore:ruleForm.passScore">
        ...
        <el-dialog :title="titleType" :visible.sync="adddialogExamVisible" width="80%">
          <examination-add-dialog v-if="adddialogExamVisible" :exam-item="examItem" @confirmAddExam="confirmAddExam" />
        </el-dialog>
      </div>
    </div>
      ...
      <div class="stepsBox3-content">
        ...
        <el-form :model="formData3" label-width="360px" label-position="left">
          ...
          <el-form-item label="2、复制考试链接发给考生，打开链接参加考试">
            ...
            <el-input v-model="formData3.examinaUrl" class="fl" placeholder="Please input" style="width:400px;max-width:100%;" />
            <el-button
              v-clipboard:copy="formData3.examinaUrl"
              v-clipboard:success="clipboardSuccess"
              class="fl"
              type="primary"
              icon="el-icon-document-copy">复制考试链接</el-button>
              <!--</el-input>-->
          </el-form-item>
...
import { addExamSendExam, examSelectExamInfo } from '@/api/hr/examManage'
  ...
  methods: {
    ...
    initData() {
      examSelectExamInfo({ id: this.$route.query.id }).then(res => {
        if (res.data.code === '0000') {
          ...
          this.ruleForm.date1 = [
            this.ruleForm.effectiveTimeStart,
            this.ruleForm.effectiveTimeEnd
          ]
        } else {
    ...
    confirmOption(val) {
      ...
      if (val.type) {
        ...
        this.ruleForm.examinabanksList = this.ruleForm.examinabanksList.concat(
          val.multipleSelection
        )
        this.ruleForm.examinabanksList = this.ruleForm.examinabanksList.reduce(
          (all, next) =>
