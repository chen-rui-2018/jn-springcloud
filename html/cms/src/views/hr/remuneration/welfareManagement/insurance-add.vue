    ...
    <el-form>
      <el-row>
        <el-col :span="12">
          <el-form-item label="参保城市：" prop="id">
            <el-input style="width: 205px;" v-model="addInsuranceData.insuredCityName" @focus="insuranceCityPageOpen"/>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="方案名称：" prop="id">
            <el-input style="width: 205px;" v-model="addInsuranceData.schemeName"/>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <div>
        ...
        <div style="margin-left: 5px; margin-top: 10px;">
          <el-table :data="addInsuranceData.socialSecurityTableData" ref="socialSecurityTable" tooltip-effect="dark" border stripe style="width: 100%" @selection-change='selectSocialSecurityRow'>
            ...
            <el-table-column label="默认基数">
              <template slot-scope="scope">
                <el-input v-model="scope.row.defaultCardinalNumber"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="可选基数范围起">
              <template slot-scope="scope">
                <el-input v-model="scope.row.optionalBaseStart"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="可选基数范围止">
              <template slot-scope="scope">
                <el-input v-model="scope.row.optionalBaseEnd"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="公司缴纳比例">
              <template slot-scope="scope">
                <el-input v-model="scope.row.corporateContributionRatio"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="个人缴纳比例">
              <template slot-scope="scope">
                <el-input v-model="scope.row.individualContributionRatio"></el-input>
              </template>
            </el-table-column>
      ...
      <div style="margin-top: 10px;">
        ...
        <div style="margin-left: 5px; margin-top: 10px;">
          <el-table :data="addInsuranceData.tableData" ref="table" tooltip-effect="dark" border stripe style="width: 100%" @selection-change='selectRow'>
            <el-table-column type="selection" width="45" align="center"></el-table-column>
            <el-table-column  label="项目" align="center"><!--:render-header="renderProvidentFundHeader"-->
              <template slot-scope="scope">
                <el-input v-model="scope.row.projectName" :disabled="accumulationFundDisabled"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="默认基数">
              <template slot-scope="scope">
                <el-input v-model="scope.row.defaultCardinalNumber" :disabled="accumulationFundDisabled"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="可选基数范围起">
              <template slot-scope="scope">
                <el-input v-model="scope.row.optionalBaseStart" :disabled="accumulationFundDisabled"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="可选基数范围止">
              <template slot-scope="scope">
                <el-input v-model="scope.row.optionalBaseEnd" :disabled="accumulationFundDisabled"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="公司缴纳比例">
              <template slot-scope="scope">
                <el-input v-model="scope.row.corporateContributionRatio" :disabled="accumulationFundDisabled"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="个人缴纳比例">
              <template slot-scope="scope">
                <el-input v-model="scope.row.individualContributionRatio" :disabled="accumulationFundDisabled"></el-input>
              </template>
            </el-table-column>
    ...
    methods: {
      ...
      initEditData(row){
        ...
        api('hr/SalaryWelfareManagement/insuranceSchemeDetailed', row).then(res => {
          if (res.data.code === '0000') {
            let insuredSchemeDetailedList = res.data.data.insuredSchemeDetailedList
      ...
      delSocialSecurityData () {
        ...
        for (let i = 0; i < this.selectSocialSecuritylistRow.length; i++) {
          let val = this.selectSocialSecuritylistRow
          // 获取选中行的索引的方法
          // 遍历表格中tableData数据和选中的val数据，比较它们的rowNum,相等则输出选中行的索引
          // rowNum的作用主要是为了让每一行有一个唯一的数据，方便比较，可以根据个人的开发需求从后台传入特定的数据
          val.forEach((val, index) => {
