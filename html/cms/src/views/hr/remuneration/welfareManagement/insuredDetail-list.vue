  ...
  <div v-loading="listLoading" class="insuredDetaildList">
    ...
    <div style="height:92px; width: 99.8%; border: 1px solid #b3d4fc">
      <span style="width: 7%;float: left;line-height: 92px;display:inline-block;text-align: center;font-size: 13px;font-weight: bold;color: blue">
        <img src="@/assets/images/left.png" style="height: 20px;width: 20px;" @click="getInsuredMonthPreMonth" alt="图片">
         {{queryMonth}}
        <img src="@/assets/images/right.png" style="height: 20px;width: 20px;" @click="getInsuredMonthNextMonth" alt="图片">
      </span>
    ...
    <!-- 表格 -->
    <el-table :data="insuredDetaildList" ref="insuredDetaildTab" border fit highlight-current-row style="width: 100%;height:100%;margin-top: 10px;">
      ...
      <el-dialog :visible.sync="stopInsuranceFormVisible" title="减员月份" width="650px">
        <el-form label-position="right" label-width="80px">
          <el-form-item label="减员月份:">
            <el-date-picker type="month" v-model="stopInsuranceFormData.insuredMonth" value-format="yyyy-MM" placeholder="" style="width: 100%;"/>
          </el-form-item>
        </el-form>
...
<script>
import {
  api, paramApi,exportExcel,getPreMonth,getNextMonth,apiGet
} from '@/api/hr/common'
...
export default {
  ...
  methods: {
    ...
    getInsuredMonthNextMonth(){
      let curMonth = this.queryMonth
      let year = curMonth.substr(0,4)
    ...
    stopInsuranceTimeSubmit(){
      let seleRow = this.$refs.insuredDetaildTab.store.states.selection[0]
    ...
    stopInsurance(){
      let seleRow = this.$refs.insuredDetaildTab.store.states.selection
      if (seleRow.length != 0){
        if (seleRow.length >1 ){
    ...
    getDetailSub(month,departmentId){
      ...
      api('hr/SalaryWelfareManagement/insuredDetailedSubsidiary', {page:1,rows:10,insuredMonth:month,departmentId:departmentId}).then(res => {
        if (res.data.code === '0000') {
          ...
          let summary = (res.data.data.personalSocialSecurity+res.data.data.companySocialSecurity)*res.data.data.insuredNumber
