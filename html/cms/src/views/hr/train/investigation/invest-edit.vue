<template>
    <el-card>
        ...
        <!-- 试题名称 -->
        <ExamTitle :isAdd = false :parentData='formData'></ExamTitle>
        <!-- 题目汇总 -->
        <div class="examBlock" v-for="(item,index) of formData.questionList" v-on:mouseover="changeActive($event)" v-on:mouseout="removeActive($event)">
            <div v-if="!item.isModify">
                <div class="examList">
                    ...
                    <div style="padding-left:26px;" v-if="item.titleType === 1 || item.titleType === 2">
                        <p style="font-size:14px;" v-for="(item2,index2) of item.titleOptionList"><span>{{upperCase[index2]}}</span>&nbsp;&nbsp;<span>{{item2.optionName}}</span></p>
                    </div>
                    <!-- <el-input v-if="item.titleType === 3" type="textarea" rows="5" v-model="item.optionName" :disabled="true" style="width:36%;"></el-input> -->
                </div>
                <div class="operateBtn"><i class="el-icon-edit-outline" @click="modifySelect(item,index)"></i><i class="el-icon-delete" @click="deleteSelect(item,index)"></i></div>
            </div>
            <!-- 修改 -->
            <div v-if="item.isModify" v-click-outside="saveFirst">
                <el-row>
                    <el-form label-width="46px">
                        <el-form-item :label="'Q'+item.Qnum" label-width="26px">
                            <el-row class="title">
                                <el-col :span="10"><el-input v-model="item.titleName"></el-input></el-col>
                        ...
                        <el-form-item v-if="item.titleType === 1 || item.titleType === 2" v-for="(item2,index2) of item.titleOptionList" :key="index2" :label="upperCase[index2]" >
                            <el-row class="option">
                                <el-col :span="9"><el-input v-model="item2.optionName"></el-input></el-col>
                                <el-col :span="2"><span class="ptionIcon"><i class="el-icon-delete" @click="deleteOption(index,index2,item2)"></i></span></el-col>
                            </el-row>
                        </el-form-item>
                        <el-form-item v-if="item.titleType === 1 || item.titleType === 2">
                            <el-row><el-col :span="9"><el-button class="addOption" @click="addOption(index)"><i class="el-icon-plus"></i>添加选项</el-button></el-col></el-row>
                        </el-form-item>
                ...
                <div class="operateBtn"><i class="el-icon-delete" @click="deleteSelect(item,index)"></i></div>
            </div>
        </div>
        <!-- 题目类型 -->
        <div class="examBlock">
            <div class="examType">
                ...
                <el-button @click="addQitem(1)"><i class="el-icon-remove-outline"></i>单选题</el-button>
                <el-button @click="addQitem(2)"><i class="el-icon-circle-check-outline"></i>多选题</el-button>
                <el-button @click="addQitem(3)"><i class="el-icon-time"></i>主观题</el-button>
            </div>
        </div>
        <!-- 操作按钮 -->
...
export default {
    ...
    methods: {
        ...
        saveSelect(item,index) {
            ...
            api('hr/train/updateQuestion', item).then(res => {
                if (res.data.code === '0000') {
                    this.$set(this.formData.questionList, index, item)
        ...
        deleteSelect(item,index) {
            ...
            this.$confirm(`删除这道题目, 是否继续?`, '删除提示', {
                ...
                api('hr/train/deleteQuestion ', data).then(res => {
                    if (res.data.code === '0000') {
