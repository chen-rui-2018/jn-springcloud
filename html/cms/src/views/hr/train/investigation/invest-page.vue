            ...
            <div v-for="(item,index) of examList" :key="index" class="examList">
              ...
              <div v-if="item.titleType === 1 || item.titleType === 2" style="padding-left:26px;">
                <p v-for="(item2,index2) of item.titleOptionList" :key="index2" style="font-size:14px;">
                  <el-radio v-if="item.titleType === 1" v-model="item.answerList[0].optionAnswer" :label="upperCase[index2]"/>&nbsp;&nbsp;
                  <el-checkbox v-if="item.titleType === 2" v-model="item.answerList[0].optionAnswer">{{ upperCase[index2] }}</el-checkbox>&nbsp;&nbsp;
                  <span>{{ item2.optionName }}</span>
                </p>
              </div>
              <el-input v-if="item.titleType === 3" v-model="item.answerList[0].subjectiveQuestionAnswer" type="textarea" rows="5"/>
            </div>
