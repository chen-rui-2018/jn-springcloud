    ...
    <el-row>
      <el-col :span="4">
        <div class="grid-content bg-purple">
          <el-radio-group v-model="listQuery.actiStatus" @change="handleFilter">
      ...
      <el-col :span="4">
        <div class="grid-content bg-purple-light">
          <el-radio-group v-model="listQuery.isIndex" @change="handleFilter">
            <el-radio-button label="">全部</el-radio-button>
        ...
        <div class="grid-content bg-purple">
          <el-form :inline="true" :model="listQuery" class="filter-bar">
            ...
            <el-form-item label="活动名称" style="line-height: 34px;">
              <el-input v-model.trim="listQuery.actiName" maxlength="20" placeholder="请输入活动名称" class="filter-item" clearable @keyup.enter.native="handleFilter"/>
            </el-form-item>
    ...
    <el-table :data="activityList" border fit highlight-current-row style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="活动时间" align="center" min-width="100">
        <template slot-scope="scope">
          ...
          <span>{{ scope.row.actiStartTime }}</span>-
          <span>{{ scope.row.actiEndTime }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column fit label="操作" align="center" width="auto" min-width="200">
        <template slot-scope="scope">
          <el-button type="text" class="operation" @click="checkDo(scope.row)">查看
          </el-button>
          <el-button v-if="scope.row.actiStatus==='1'" type="text" class="operation" @click="editDo(scope.row)">编辑
          </el-button>
          <el-button v-if="scope.row.actiStatus==='2'&& scope.row.isApply==='1'" type="text" class="operation" @click="cancelApply(scope.row)">停止报名
          </el-button>
          <!-- <el-button
          ...
          <el-button v-if="scope.row.actiStatus==='2'" type="text" class="operation" @click="handleCancelActivity(scope.row)">取消活动
          </el-button>
          <!-- v-if="scope.row.actiStatus==='3'||scope.row.actiStatus==='4'" 查看判断条件，改成全部都可以查看-->
          <el-button v-if="scope.row.actiStatus==='1'||scope.row.actiStatus==='3'||scope.row.actiStatus==='4'" type="text" class="operation" @click="handleDeleteActivity(scope.row)">删除
          </el-button>
          <el-button v-if="scope.row.isSendMessage==='1'" type="text" class="operation" @click="handlePushMessage(scope.row)">推送
          </el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    handlePushMessage(row) {
      this.secondaryConfirmation(
        ...
        '推送消息成功',
        paramApi,
        `${this.GLOBAL.parkUrl}activity/sendMsgForActivate`,
        row.id,
        'activityId'
      )
    },
    ...
    recoverApply(row) {
      this.secondaryConfirmation(
        ...
        '恢复报名成功',
        api,
        `${this.GLOBAL.parkUrl}activity/updateActivityApply`,
        { activityId: row.id, actiStatus: '1' },
        'post'
      )
    },
    // 停止报名
    cancelApply(row) {
      this.secondaryConfirmation(
        ...
        api,
        `${this.GLOBAL.parkUrl}activity/updateActivityApply`,
        { activityId: row.id, actiStatus: '0' },
        'post'
      )
    },
    // 批量删除
    handleBatchDeleteActivity() {
      if (this.checkActivity.length > 0) {
        this.secondaryConfirmation(
          ...
          '删除成功',
          paramApi,
          `${this.GLOBAL.parkUrl}activity/deleteActivity`,
          this.checkActivity,
          'activityId'
        )
      } else {
    ...
    handleDeleteActivity(row) {
      if (row.actiStatus !== '1') {
        ...
        this.secondaryConfirmation(
          ...
          '删除成功',
          paramApi,
          `${this.GLOBAL.parkUrl}activity/deleteDraftActivity`,
          row.id,
          'activityId'
        )
      }
    ...
    handleCancelActivity(row) {
      this.secondaryConfirmation(
        ...
        '取消成功',
        paramApi,
        `${this.GLOBAL.parkUrl}activity/cancelActivity`,
        row.id,
        'activityId'
      )
    },
    ...
    handleApplyNum(row) {
      ...
      this.$router.push({ path: `applyActivityList?id=${row.id}&applyCheck=${row.applyCheck}` })
    },
    // 编辑
    editDo(row) {
    ...
    checkDo(row) {
      ...
      this.$router.push({
        path: `activityCheck`,
        query: { activityId: row.id, disabled: true }
      })
    },
    // 项目初始化
    initList() {
      ...
      api(
        `${this.GLOBAL.parkUrl}activity/selectActivityList`,
        this.listQuery,
        'post'
      ).then(res => {
        if (res.data.code === this.GLOBAL.code) {
