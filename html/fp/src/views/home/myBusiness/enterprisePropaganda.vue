          ...
          <el-table-column label="操作" align="center" >
            <template slot-scope="scope">
              ...
              <el-button
                ...
                type="text"
               class="greenColor"
                @click="toPropagandaDetails(scope.row)"><span>详情</span>
              </el-button>
               <el-button
                size="mini"
                type="text"
                @click="cancelApprove( scope.row)" class="redColor"><span>撤消申请</span>
              </el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
 ...
 this.$router.push({name:'propagandaDetails',query:{propagandaId :row.id}})
    },
    ...
    // 初始化页面
     initList(){
      ...
      this.api.get({
        ...
        data: {page:this.page,rows:this.rows,searchContent:this.searchContent,approvalStatus:this.approvalStatus,needPage:'1'},
        // dataFlag:true,
        callback: function(res) {
          if (res.code == "0000") {
            console.log(res)
