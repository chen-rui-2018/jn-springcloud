...
public class GoodsOrderController {
    ...
    private Map createPayOrder2(GoodsOrder goodsOrder, Map<String, Object> params) {
        JSONObject paramMap = new JSONObject();
        //paramMap.put("mchId", mchId);                       // 商户ID
        //支付类型
    ...
    private Map createPayOrder(GoodsOrder goodsOrder, Map<String, Object> params) {
        ...
        payOrder.setPayType(params.get("payType").toString());
        payOrder.setMchOrderNo(goodsOrder.getGoodsOrderId());
        payOrder.setChannelId(params.get("channelId").toString());
        payOrder.setAmount(new Long(goodsOrder.getAmount()));
        payOrder.setSubject("商品测试标题");
        payOrder.setBody("阳的测试消费");
        payOrder.setNotifyUrl(notifyUrl);
        payOrder.setExtra("{\"qr_pay_mode\":\"4\",\"qrcode_width\":\"400\"}");

        // 签名
        String reqSign = PayDigestUtil.getSign(BeanToMap.toMap(payOrder), reqKey);
        payOrder.setSign(reqSign);



        String reqData = "params=" + JSONObject.toJSONString(payOrder);
        System.out.println("请求支付中心下单接口,请求数据:" + reqData);
        String url = baseUrl + "/pay/create_order?";
        String result = XXPayUtil.call4Post(url + reqData);
        System.out.println("请求支付中心下单接口,响应数据:" + result);
    ...
    public void payNotify(HttpServletRequest request, HttpServletResponse response ,@ModelAttribute PayOrder model) throws Exception {
        ...
        String testSign = PayDigestUtil.getSign(BeanToMap.toMap(model), resKey,"sign");
        if(testSign.equals(paramMap.get("sign").toString())){
            System.out.println("就是可以咯=======================================");
        }
    ...
    public static void main(String[] args) {
        ...
        String    mchId1 = MchPayTypeEnum.getMchPayTypeEnum("0001").getMchId();
        String    mchId2 = MchPayTypeEnum.getMchPayTypeEnum("0002").getMchId();
        String    mchId3 = MchPayTypeEnum.getMchPayTypeEnum("0003").getMchId();
        String    mchId4 = MchPayTypeEnum.getMchPayTypeEnum("00013") == null ? "" : MchPayTypeEnum.getMchPayTypeEnum("00013").getMchId();



        System.out.println("mchId1 :"+ mchId1);
        System.out.println("mchId2 :"+ mchId2);
        System.out.println("mchId3 :"+ mchId3);
        System.out.println("mchId4 :"+ mchId4);

        if(StringUtils.isBlank(mchId4)){
            System.out.println("就是空咯");
        }
    }
}