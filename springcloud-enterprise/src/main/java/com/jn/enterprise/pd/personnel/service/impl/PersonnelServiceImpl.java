...
public class PersonnelServiceImpl implements PersonnelService {
    ...
    @Override
    public PaginationData<List<PersonnelProject>> selectByPersonnelProjectList(PersonnelProject personnelProject) {
        Page<Object> objects = PageHelper.startPage(personnelProject.getPage(), personnelProject.getRows());
        ...
        if(StringUtils.isNotEmpty(personnelProject.getCreditCode())) {  criteria.andCreditCodeLike('%'+personnelProject.getCreditCode()+'%');}
        if(StringUtils.isNotEmpty(personnelProject.getType())) {  criteria.andTypeEqualTo(personnelProject.getType());}
        if(StringUtils.isNotEmpty(personnelProject.getStepId().toString())) {  criteria.andStepIdEqualTo(personnelProject.getStepId());}
        ...
        return new PaginationData(tbPersonnelProjectMapper.selectByExample(declarationCriteria), objects.getTotal());
    }
}
